...
async def post_register_viewmodel(request: Request) -> ViewModel:  
    ...
    #:
        ...
        error, error_msg = True, f'Endereço de email {email} já foi registado!'  
    else: 
...
async def post_login_viewmodel(request: Request) -> ViewModel:
    ...
    #:
        ...
        error, error_msg = True, 'Senha inválida!'
    #:
    elif not (student := student_service.autheticate_student_by_email(email, password)):
        error, error_msg = True, 'Dados do utilizador inválidos!'
    #:
    else:
         error, error_msg = False, ''
